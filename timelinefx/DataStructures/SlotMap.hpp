...
#ifndef SlotMap_THROW_EXCEPTION
#include <stdexcept>
#define SlotMap_THROW_EXCEPTION(type, ...) throw type(__VA_ARGS__)
#endif
...
public:
    ...
    using key_size_type = decltype(SlotMap::get_index(std::declval<Key>()));
    using key_generation_type = decltype(SlotMap::get_generation(std::declval<Key>()));
    ...
    reference at(const key_type& key)
    {
        ...
        {
            SlotMap_THROW_EXCEPTION(std::out_of_range, "at");
        }
        return *value_iter;
    }
...
void swap(SlotMap<T, Key, Container>& lhs, SlotMap<T, Key, Container>& rhs)
{
    lhs.swap(rhs);
}
