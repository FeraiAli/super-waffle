...
class EventMap<void(Args...)>
{
public:
    using slot_type = std::function<void(Args...)>;
    using slot_container = SlotMap<slot_type>;
    ...
    template <typename T,
              typename = typename std::enable_if<
                  !std::is_same<EventMap, typename std::decay<T>::type>::value>::type>
    ...
    template <typename T,
              typename = typename std::enable_if<
                  !std::is_same<EventMap, typename std::decay<T>::type>::value &&
    ...
    /// Emits the EventMaps you wish to send to the call-backs
    ...
    /// Emits EventMaps you wish to send to call-backs
    /// \param args The arguments to emit to the slots connected to the signal
    /// \note
    ...
    bool operator==(const EventMap& s) const
    {
...
private:
    ...
    {
        auto it = std::find_if(std::begin(slots_),
                               std::end(slots_),
