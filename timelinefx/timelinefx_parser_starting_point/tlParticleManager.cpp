...
tlParticleManager::~tlParticleManager()
{
	delete m_FontAtlas[0];
	delete m_FontAtlas[1];
...
void tlParticleManager::DrawParticles( void * eventData )
{
	...
	if ( camera->m_DrawMask & m_DrawMask )
	{
		...
		// clear font buffers. We're about to refill them.
		for (int i=0; i<2; ++i)
		{
			m_FontAtlas[ i ]->Flush();
		}
		...
		// Finally, add the fonts to the camera's drawlist
		for (int i=0; i<2; ++i)
		{
			Font * font = m_FontAtlas[ i ];

			// anything to draw?
			if ( font->m_Mesh->m_IndicesDrawCount )
			{
				drawlist->AddFont( font, (Matrix*)&MATRIX_IDENTITY );
			}
		}
	}
}	
